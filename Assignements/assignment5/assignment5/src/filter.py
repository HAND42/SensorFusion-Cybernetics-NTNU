...
class FilterPDA:
    ...
    def gate_zs(self,
        ...
        for i, z in enumerate(zs):
            condition = z_est_pred.mahal_dist(z) < self.gate**2
            if condition:
                zs_gated.append(z)
                gated_indices.add(i)
    ...
    def get_assoc_probs(self, z_est_pred: MultiVarGauss[MeasPos],
                        zs_gated: Sequence[MeasPos]) -> np.ndarray:
        ...
        assoc_probs = np.zeros(len(zs_gated) + 1)

        assoc_probs[0] = lamb * (1 - P_D)
        for i, z in enumerate(zs_gated):
            assoc_probs[i+1] = P_D * z_est_pred.pdf(z)

        assoc_probs /= np.sum(assoc_probs)

        # TODO remove this
        # assoc_probs = filter_solu.FilterPDA.get_assoc_probs(
        #     self, z_est_pred, zs_gated)
    ...
    def get_estimates(self,
        ...
        Hint: Use self.ekf"""
        x_ests = []
        gauss_ak0 = MultiVarGauss(x_est_pred.mean, x_est_pred.cov)
    ...
    def step(self,
        ...
        x_est_pred, z_est_pred = self.ekf.pred(
            x_est_prev, dt)  # Hint: (7.16) and (7.17)
        gated_indices, zs_gated = self.gate_zs(z_est_pred, zs)  # Hint: (7.3.5)
        assoc_probs = self.get_assoc_probs(
            z_est_pred, zs_gated)  # Hint (Corollary 7.3.3)
        # Hint: (7.20) and (7.21)
        x_ests = self.get_estimates(x_est_pred, z_est_pred, zs_gated)
        x_est_upd_mixture = GaussianMixture(
            assoc_probs, x_ests)    # TODO Hint: (7.3.6)
