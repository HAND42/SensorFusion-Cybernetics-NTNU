...
class ExtendedKalmanFilter:
    ...
    def step(self,
        ...
        # print(meas)
        # print(meas_pred.mean)
        innovation = (meas.value.T - meas_pred.mean.T)

        state_upd_mean = state_pred.mean + kalman_gain@innovation
        state_upd_cov = state_pred.cov - kalman_gain@H@state_pred.cov

        state_upd = MultiVarGauss(state_upd_mean, state_upd_cov)

        # print("our state_updcov", state_upd.cov)
        # print("our state_updmean", state_upd.mean)

        # TODO replace this with own code
        # state_upd, state_pred, meas_pred = ekf_solu.ExtendedKalmanFilter.step(
        #     self, state_old, meas)

        # print("truth state_updcov", state_upd.cov)
        # print("truth state_updmean", state_upd.mean)
