...
def get_nis(meas_pred: MultiVarGauss, meas: Measurement2d) -> float:
    ...
    nis = (meas.value.T - meas_pred.mean.T).T @ np.linalg.inv(
        meas_pred.cov)@(meas.value.T - meas_pred.mean.T)
...
def get_nees(state_est: MultiVarGauss, x_gt: np.ndarray):
    ...
    NEES = (state_est.mean.T -
            x_gt.T).T @ np.linalg.inv(state_est.cov)@(state_est.mean.T - x_gt.T)

    # print("our_ness", NEES)

    # TODO replace this with own code
    # NEES = analysis_solu.get_nees(state_est, x_gt)
    # print("truth_ness", NEES)
    return NEES
