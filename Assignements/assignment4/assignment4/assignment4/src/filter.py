...
class FilterIMM:
    ...
    def calculate_mixings(self, x_est_prev: GaussianMixture[StateCV],
        ...
        The output should be on the following form:
        $mixing_probs[s_{k-1}, s_k] = \mu_{s_{k-1}|s_k}$
        """

        pi_mat = self.dynamic_model.get_pi_mat_d(dt)  # the pi in (6.6)
        prev_weights = x_est_prev.weights  # \mathbf{p}{k-1}

        num_modes = len(prev_weights)
        mixing_probs = np.zeros_like(pi_mat)

        for s_k in range(num_modes):
            for s_k_minus_1 in range(num_modes):
                mixing_probs[s_k_minus_1, s_k] = pi_mat[s_k_minus_1,
                                                        s_k] * prev_weights[s_k_minus_1]

       # Normalize the mixing probabilities for each s_k
        mixing_probs_sum = np.sum(mixing_probs, axis=0)
        mixing_probs /= mixing_probs_sum
    ...
    def mode_match_filter(self,
        ...
        ekfs = [EKF(self.dynamic_model.models[i], self.sensor_model)
                for i in range(len(self.dynamic_model.models))]

        for i, x_prev in enumerate(moment_based_preds):
            out_ekf = ekfs[i].step(x_prev, z, dt)
            ekf_outs.append(out_ekf)

        # print("mine", ekf_outs[1])

        # TODO remove this
        # ekf_outs = filter_solu.FilterIMM.mode_match_filter(
        #     self, moment_based_preds, z, dt)

        # print("truth", ekf_outs[1])
    ...
    def update_probabilities(self,
        ...
        weights_pred = pi_mat.T @ weights.T
        z_probs = [MultiVarGauss(ekf.z_est_pred.mean, ekf.z_est_pred.cov, dt).pdf(z)
                   for ekf in ekf_outs]

        weights_upd = z_probs * weights_pred
        # Normalize to ensure it sums to 1
        weights_upd /= np.sum(weights_upd)

        # print("weights_upd ourr", weights_upd)

        # TODO remove this
        # weights_upd = filter_solu.FilterIMM.update_probabilities(
        #     self, ekf_outs, z, dt, weights)

        # print("weights_upd tru", weights_upd)
    ...
    def step(self,
        ...
        weights_upd = self.update_probabilities(
            ekf_outs, z, dt, x_est_prev.weights)
