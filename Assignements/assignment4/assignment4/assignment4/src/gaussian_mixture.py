...
class GaussianMixture(Generic[S]):
    ...
    def mean(self):
        ...
        mean = np.sum(
            self.weights[i] * self.gaussians[i].mean for i in range(len(self.gaussians)))
        # print("our", mean)
        # TODO remove this
    ...
    def cov(self):
        ...
        mean_hat = self.mean()
        P_tilde = np.sum(self.weights[i] * self.gaussians[i].mean @
                         self.gaussians[i].mean.T for i in range(len(self.gaussians))) - mean_hat @ mean_hat.T
        cov = np.sum(
            self.weights[i] * self.gaussians[i].cov for i in range(len(self.gaussians))) + P_tilde

        # print("mycov", cov)
        # print("ourcov", type(cov))

        # TODO remove this
        # cov = gaussian_mixture_solu.GaussianMixture.cov(self)
        # print("trthucov", cov)
        # print("trthcov", type(cov))
        return cov
